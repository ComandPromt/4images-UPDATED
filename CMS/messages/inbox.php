...
while($fila = mysqli_fetch_row($consulta)){
	...
	print '<tr><td style="color:#7a4a0f;font-weight:bold;">'.$fila[0].'</td><td>'.$fila[1].'</td><td style="color:#074E20;"><img class="imgRedonda" style="height:40px;width:40px;margin-right:20px;" src="../'.$avatar.'"/>'.$fila[2].'</td></tr>
...
<a title="Emoji" style="padding-left:10px;" data-toggle="modal" data-target="#emojis">
          ...
          <form action="'.$_SERVER['PHP_SELF'].'?msg_id='.$id_mensajes[$x].'&destinatario='.$destinatarios[$x].'&asunto='.$asuntos[$x].'" method="post">
		     ...
		     <textarea style="margin-top:20px;" class="input" id="mensaje" name="msg_reply" type="text"></textarea>
			 <input class="negrita" style="margin-top:20px;" type="submit" value="'.ver_dato('reply', $GLOBALS['idioma']).'" name="respuesta" />
        </form>
