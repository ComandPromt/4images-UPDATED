...
if ($fila[0] > 0) {
    ...
    while ($fila = mysqli_fetch_row($consulta)) {
		...
		print '<tr>
					<td style="color:#7a4a0f;font-weight:bold;">' . $fila[0] . '</td>

					<td>' . $fila[1] . '</td>

					<td>
						<img style="height:40px;width:40px;margin-right:20px;" src="../img/user.png"/>'.
						 $fila[2] . '
					</td>
		...
		<tr>
			<td colspan="3">
				<button type="button" style="height:50px;" data-toggle="modal" data-target="#reply'. $longitud . '">
				   ' . ver_dato('reply', $GLOBALS['idioma']) . '</button>
			</td>
						...
						<div class="modal-content ">
							...
							<div class="modal-body transparente">
        						...
        						<form action="' . $_SERVER['PHP_SELF'] . '?msg_id=' . $id_mensajes[$x] . '&destinatario=' . $destinatarios[$x] . '&asunto=' . $asuntos[$x] . '" method="post">
									...
									<input style="margin-top:20px;" type="submit" value="'.
									 ver_dato('reply', $GLOBALS['idioma']) . '" name="respuesta" />
        						</form>
