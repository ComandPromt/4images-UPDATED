...
function categoria_link(){
	...
	if($cat>0){
		...
		if($num_hijos==0){
			...
			print '<div style="margin-auto;" class="flotar_izquierda espacio_derecha">';
			...
			while($datos_categoria = mysqli_fetch_row($imagenes_links)){
				...
				if( $datos_categoria[4]==1 || ($logueado && $datos_categoria[4]==2) ){
					...
					print '	<a title="'.$datos_categoria[1].'" href="details.php?image_id='.$datos_categoria[0].'">
						...
						<img style="margin-top:20px;" alt="'.$datos_categoria[1].'" class="rounded icono" src="data/media/'.$datos_categoria[2].'/'.$datos_categoria[3].'"/>
					</a>';
				}
...
function registrar(){
	...
	print '
	<div style="padding-bottom:40px;margin-left:-60px;margin-top:-20px;clear:both;" class="flotar_izquierda">
	...
	if($geo->lat>0){
		...
		print '<script>
			...
			f.getDate()<=21 && f.getMonth()==11 ){
				...
				function winSize(){
					winWidth=(moz)?window.innerWidth-180:document.body.clientWidth-180;winHeight=(moz)?window.innerHeight+500:document.body.clientHeight+500;
				}
				...
				function fallObject(num,vari,nu){
					...
					if(nu==1){
						document.write(\'<img id="fO\'+i+\'" class="fijo" src="\'+fallObjects[vari][0]+\'">\'); 
					}
				}
...
function comprobar_cookie($ruta=""){
	...
	if(!isset($_COOKIE['pass']) || !isset($_COOKIE['4images_userid']) || (int)$_COOKIE['4images_userid']<=0 || empty($_COOKIE['pass']) || empty($_COOKIE['4images_userid'])){
			redireccionar($ruta.'index.php');	
	}
					...
					<div class="modal-content ">
						...
						<div class="modal-body" >
							...
							<form id="limpiar_mensajes">
								...
								<p>
									<a href="clear.php?delete=1"><span>'.ver_dato('clear_inbox', $GLOBALS['idioma']).'</span></a>
								</p>
								
								<p>
									<a href="clear.php?delete=2"><span>'.ver_dato('clear_outbox', $GLOBALS['idioma']).'</span></a>
								</p>
			...
			if($recuento>1){
				...
				for($x=0;$x<$recuento-1;$x++){
					...
					print '<tr style="border:none;">
							...
							<td style="font-size:1.8em;border:none;font-weight:bold;">'.$tercera_reja.'</td>
						</tr>';
...
function menu_lateral($ruta = "",$registro=false,$verImagenAleatoria=false){
...
<?php
	...
	if(isset($_GET['l']) && $_GET['l']=='yes' || isset($_COOKIE['4images_userid']) && $_COOKIE['4images_userid']=="-1" || !isset($_COOKIE['4images_userid']) || $_COOKIE['4images_userid']=="-1"){
	...
	if($verImagenAleatoria &&  logueado()){
