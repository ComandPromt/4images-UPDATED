...
if(compare_passwords($_POST['password'], $fila[0])){
	...
	$_SESSION['logueado']=true;
							...
							if(isset($_FILES['upload']['name']) && $_SESSION['logueado']){
				
				
				for($i=0; $i<count($_FILES['upload']['name']); $i++) {
					
					$extension=strtolower(substr($_FILES['upload']['name'][$i],-3));
					
					if($extension=='peg'){
						$_FILES['upload']['name'][$i]=substr($_FILES['upload']['name'][$i],0,-4).'jpg';
					}
					
					$fichTemporal = $_FILES['upload']['tmp_name'][$i];
				
					if ($fichTemporal != ""){
						$destino = '../data/media/1/'.$_FILES['upload']['name'][$i];
